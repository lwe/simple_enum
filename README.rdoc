...
Working with database backed values, the only requirement to enable this is that _a)_ either a field name +name+ exists
or _b)_ a custom method to convert an object to a symbol named +to_enum_sym+ (for general uses overriding +to_enum+ is perfectly fine):
