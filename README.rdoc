...
Working with database backed values, the only requirement to enable this is that <em>a)</em> either a field name +name+ exists
or <em>b)</em> a custom method to convert an object to a symbol named +to_enum_sym+ (for general uses overriding +to_enum+ is perfectly fine):
    ...
    t = BankTransaction.new(:status_cd = 1)
    t.status # => #<Status id: 1, name: "active">
    
    # and pulling fun tricks:
    BankTransaction.active # => 1
    BankTransaction.active(true) # => #<Status id: 1, name: "active">
